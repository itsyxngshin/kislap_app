...
class _AddDeviceScreenState extends ConsumerState<AddDeviceScreen> {
  ...
  static const List<Map<String, dynamic>> _fallbackCatalog = [
    {'id': 1, 'category': 'Cooling', 'appliance_name': 'Inverter AC (1.0 HP)', 'preset_wattage': 750.0},
    {'id': 2, 'category': 'Cooling', 'appliance_name': 'Non-Inverter AC (1.0 HP)', 'preset_wattage': 1000.0},
    {'id': 3, 'category': 'Cooling', 'appliance_name': 'Electric Fan', 'preset_wattage': 65.0},
    {'id': 4, 'category': 'Entertainment', 'appliance_name': 'LED TV (32")', 'preset_wattage': 50.0},
    {'id': 5, 'category': 'Entertainment', 'appliance_name': 'LED TV (43")', 'preset_wattage': 80.0},
    {'id': 6, 'category': 'Kitchen', 'appliance_name': 'Inverter Refrigerator', 'preset_wattage': 120.0},
    {'id': 7, 'category': 'Kitchen', 'appliance_name': 'Standard Refrigerator', 'preset_wattage': 150.0},
    {'id': 8, 'category': 'Kitchen', 'appliance_name': 'Microwave', 'preset_wattage': 1000.0},
    {'id': 9, 'category': 'Kitchen', 'appliance_name': 'Rice Cooker', 'preset_wattage': 400.0},
    {'id': 10, 'category': 'Laundry', 'appliance_name': 'Washing Machine', 'preset_wattage': 500.0},
    {'id': 11, 'category': 'Laundry', 'appliance_name': 'Iron', 'preset_wattage': 1000.0},
    {'id': 12, 'category': 'Lighting', 'appliance_name': 'LED Bulb', 'preset_wattage': 9.0},
    {'id': 13, 'category': 'Lighting', 'appliance_name': 'Fluorescent Tube', 'preset_wattage': 20.0},
    {'id': 14, 'category': 'Computing', 'appliance_name': 'Laptop', 'preset_wattage': 65.0},
    {'id': 15, 'category': 'Computing', 'appliance_name': 'Desktop PC', 'preset_wattage': 250.0},
  ];
  ...
  Future<void> _loadPresets() async {
    ...
    try {
      ...
      // If DB fails on first boot, it safely falls back to the hardcoded list
    }
  ...
  void _saveDevice() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(isPh ? 'Matagumpay na naidagdag ang $_quantity na gamit!' : '$_quantity device(s) added successfully!'), backgroundColor: Colors.green));
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      return Scaffold(backgroundColor: Theme.of(context).scaffoldBackgroundColor, body: const Center(child: CircularProgressIndicator(color: AppColors.appYellow)));
    }
    ...
    return Container(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(isPh ? 'Magdagdag ng Gamit' : 'Add Appliance', style: TextStyle(color: textColor, fontWeight: FontWeight.bold, fontSize: 18)),
        ),
        body: SafeArea(
                    ...
                    children: [
                      if (_currentMode != ApplianceInputMode.free) ...[
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return GestureDetector(
                                onTap: () => setState(() { _selectedCategory = cat; _selectedPreset = null; _customNameController.clear(); }),
                                child: AnimatedContainer(
                                  duration: const Duration(milliseconds: 200),
                                  margin: const EdgeInsets.only(right: 8),
                                  padding: const EdgeInsets.symmetric(horizontal: 16),
                                  alignment: Alignment.center,
                                  decoration: BoxDecoration(color: isSelected ? AppColors.appYellow : surfaceColor, borderRadius: BorderRadius.circular(20), border: Border.all(color: isSelected ? AppColors.appYellow : hintColor.withOpacity(0.2))),
                                  child: Text(cat, style: TextStyle(color: isSelected ? Colors.black87 : textColor, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal, fontSize: 13)),
                                ),
                              );
                            },
                          ),
                        ...
                        DropdownButtonFormField<Map<String, dynamic>>(
                          ...
                          isExpanded: true,
                          items: filteredPresets.map((preset) => DropdownMenuItem<Map<String, dynamic>>(value: preset, child: Text('${preset['appliance_name']} (${preset['preset_wattage']}W)', style: TextStyle(color: textColor, fontSize: 15)))).toList(),
                      ...
                      TextField(
                        ...
                        decoration: _inputDecoration(surfaceColor, hintColor, Icons.schedule, iconColor: Colors.greenAccent).copyWith(hintText: isPh ? 'Oras kada araw' : 'Hours per day', suffixText: 'hrs', suffixStyle: TextStyle(color: hintColor)),
                        onChanged: (_) => setState(() {}),
                      ),
                    ],
                  ),
