...
class _AnalysisScreenState extends ConsumerState<AnalysisScreen> {
      ...
      body: Container(
        ...
        child: SafeArea(
          ...
          child: SingleChildScrollView(
            // Bottom padding set to 120 to ensure the floating navigation bar doesn't overlap content
            padding: const EdgeInsets.only(left: 24, right: 24, top: 20, bottom: 120),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // NEW: Native Header matching the other tabs
                Text(
                ...
                // 3. THE MATH GUIDE ACCORDION
                GestureDetector(
                  onTap: () => setState(() => _showFormulas = !_showFormulas),
                  child: Container(
                    padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(
                      color: AppColors.appYellow.withValues(alpha: 0.05),
                      borderRadius: BorderRadius.circular(16),
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Row(
                              children: [
                                const Icon(Icons.calculate_outlined, color: AppColors.appYellow, size: 22),
                                const SizedBox(width: 12),
                                Text(
                                  isPh ? 'PAANO KINAKALKULA NG KISLAP' : 'HOW KISLAP COMPUTES',
                                  style: const TextStyle(color: AppColors.appYellow, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.0),
                                ),
                              ],
                            ),
                        ...
                        if (_showFormulas) ...[
                          const SizedBox(height: 16),
                          Divider(color: AppColors.appYellow.withValues(alpha: 0.2)),
                          const SizedBox(height: 16),
                          _buildMathRow(
  ...
  Widget _buildMathRow(String title, String desc, String formula, Color textColor, Color hintColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        Container(
          ...
          child: Text(formula, style: const TextStyle(color: Colors.greenAccent, fontSize: 12, fontFamily: 'monospace', fontWeight: FontWeight.bold)),
        ),
      ],
    );
  }
