...
class _AddDeviceScreenState extends ConsumerState<AddDeviceScreen> {
  ...
  void _saveDevice() async {
    if (_selectedPreset == null || _customNameController.text.trim().isEmpty || _hoursController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please complete all fields.'), backgroundColor: AppColors.adminRed));
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Device added and schedule optimized!'), backgroundColor: Colors.green));
      }
    } catch (e) {
      if (mounted) {
      ...
      child: Scaffold(
        ...
        body: FutureBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator(color: AppColors.appYellow));
            if (!snapshot.hasData || snapshot.data!.isEmpty) return Center(child: Text('No appliance presets found.', style: TextStyle(color: hintColor)));
                ...
                children: [
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('APPLIANCE TYPE (URI NG GAMIT)', style: TextStyle(color: AppColors.appYellow, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                        ...
                        Text('CUSTOM IDENTIFIER (PANGALAN NG GAMIT)', style: TextStyle(color: AppColors.appYellow, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                        const SizedBox(height: 10),
                        TextField(
                        ...
                        Text('BASELINE USAGE (ORAS NG PAGGAMIT)', style: TextStyle(color: AppColors.appYellow, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                        const SizedBox(height: 10),
                        TextField(
