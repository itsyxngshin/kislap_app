...
class _GuestSetupScreenState extends ConsumerState<GuestSetupScreen> {
  ...
  // NEW: Calculates the previous month (M-1) dynamically
  ...
  Future<void> _submitGuestSetup() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error saving setup: $e'), backgroundColor: AppColors.adminRed));
      }
    } finally {
                ...
                child: Stepper(
                          ...
                          Expanded(
                            child: FilledButton(
                              ...
                              child: _isLoading
                                  ...
                                  : Text(isLastStep ? (isPh ? 'Pumasok sa Dashboard' : 'Enter Dashboard') : (isPh ? 'Magpatuloy' : 'Continue'), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                            ),
                          ),
                          if (_currentStep > 0) ...[
                            const SizedBox(width: 12),
                            TextButton(
                              ...
                              child: Text(isPh ? 'Bumalik' : 'Back', style: TextStyle(color: hintColor, fontSize: 16)),
                            ),
                          ]
                        ],
                      ),
                  ...
                  steps: [
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 4),
                                Text(isPh ? 'Suriin ang iyong electric bill noong nakaraang ${_getPreviousBillingMonth(true)} para sa eksaktong ₱/kWh.' : 'Check your electric bill from ${_getPreviousBillingMonth(false)} for the exact ₱/kWh rate.', style: TextStyle(color: hintColor, fontSize: 12, height: 1.4)),
                                const SizedBox(height: 16),
                    ...
                    Step(
                      ...
                      subtitle: Text(isPh ? 'Antas ng paggamit (kVA scale)' : 'Sets the kVA scale', style: TextStyle(color: hintColor, fontSize: 12)),
